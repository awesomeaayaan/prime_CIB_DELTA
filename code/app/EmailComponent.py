...
class EmailComponent(QRComponent):
        ...
        if result == 'OK':
            ...
            for a in byte_mail_list:
                ...
                if start_index != -1 and end_index != -1:
                    email_address = sender[start_index +1:end_index]
                    sender_list.append(email_address)
                ...
                for part in msg.walk():
                    ...
                    if filename:
                        dir_path = os.path.join(Constants.cib_common_path,'downloaded_attachment')
                        filepath = os.path.join(dir_path,filename)
                        with open(filepath, 'wb') as f:
                            f.write(part.get_payload(decode=True))
                        display(f'Download attachment:{filename}')
            display(f'Sender list is {sender_list}')
            # sender_list = ['meghnath.sonar@pcbl.com.np','bhakti.niraula@pcbl.com.np','geha.joshi@pcbl.com.np']
            self.db.update_status_of_email_table(sender_list)
            display('Successfully updated the email table ')
            # return filename, send        
    def follow_up_email_with_no_reply_from_branch(self):
        email_id = self.db.get_the_value_with_status_Zero()
        display(f'Not replying the CIB Screening mail id ==>{email_id}')
        self._get_vault()
        ...
        # display(f'Sending File list of the summary data for compliance is {self.sending_file_list}')
        self.body = ''' Please Reply the mail that is send for the CIB Screening of Blacklist Match or Not match''' 
        ...
        self.mail.send_message(
            ...
            subject=f'Please attach the CIB screening report and send to us',
            body=self.body,
            # attachments=self.sending_file_list,
            html=True
        )
        display('Successfully send mail to the respective branch ')
    ...
    def follow_up_mail(self,branch_value,attachments_path):
        ...
        df_email_file = pd.read_excel(os.path.join(Constants.cib_common_path,'Mail Id Details.xlsx'))
    ...
    def summary_mail_to_compliance(self):
        self._get_vault()
        self._set_smtp_creds()

        self.mail = ImapSmtp(smtp_server=self.server, smtp_port=int(self.port))
        self.mail.authorize_smtp(account=self.account, password=str(self.password), smtp_server=self.server, smtp_port=int(self.port),)
        directory = os.path.join(Constants.cib_common_path,'Compliance')
        for file in os.listdir(directory):
            if file.endswith('.xlsx'):
                display(f'file is {file}')
                file_path = os.path.join(directory,file)
                self.sending_file_list.append(file_path)
       
        display(f'Sending File list of the summary data for compliance is {self.sending_file_list}')
        self.body = ''' This is the Summary Report for the compliance department''' 
        # ) sushil.kc@pcbl.com.np
        ...
        self.mail.send_message(
            ...
            # subject=self.subject,
            subject=f'CIB Summary Report for the compliance',
            body=self.body,
            attachments=self.sending_file_list,
            html=True
        )
    ...
    def _authmail_and_send(self):
        ...
        try:
            ...
            # df_report.to_excel('ramkrishna.xlsx',index=True)
            ...
            display(f'Branch Name is {branch_name}')
            # status = '0'
            self.db.insert_into_email_table(receiver_mail,branch_name,status='0')
            display(f'Insert {receiver_mail} to the database')
            print(receiver_mail)
            receiver_branch_code = df['BranchCode'].iloc[0]
            display(f'This is a branch code of reciver email {receiver_branch_code} and this is branchcode from file{branch_value}')
            current_datetime = datetime.now()
