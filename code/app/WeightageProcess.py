...
# from PCBLPortal import PCBLPortalComponent
# from ConfigComponent import ConfigComponent
from Errors import DataNotFoundError, MultipleDataError, FlieAlreadyReadError
...
class WeightageProcess(QRProcess):
    def __init__(self):
        super().__init__()
        # self.config = ConfigComponent(Utils.get_local_working_directory())
        # self.register(self.config)

        run_item: QRRunItem = QRRunItem(is_ticket=False)
        self.notify(run_item)
        # self.config.set_config_file()

        # self.ftp = FTPComponent(
        #     local_download_path=BotVariable.DOWNLOAD_PATH_SOURCE,
        #     ftp_working_dir=BotVariable.FTP_WORKING_FOLDER
        #     )
        self.db = DBComponent()
        self.file = FileComponent()
        # self.email = EmailComponent()
        # self.pcbl = PCBLPortalComponent()
    ...
    def before_run_item(self, *args, **kwargs):
        ...
        item: dict = args[0] #dabase bata cib ko  incremental data aako 
        logger = run_item.logger
        try:
            ...
            self.exe_item =  self.file.collect_user_reqiured_data(item)
    ...
    def execute_run_item(self, *args, **kwargs):
        ...
        # item = {'account_nature': 'individual','name': 'JAISARI RANA', 'fathername': 'MODAR RANA', 'search_list': ['name', 'fathername']}
        # item = {'account_nature': 'institutions','name': 'NEW BHATTARAI TRADERS', 'fathername': '','dob':'','citizenship_no':'','pan_no':'603102202','registration_no':'778/073/074', 'search_list': ['name', 'pan_no','registration_no']}
        # item = {'account_nature': 'individual', 'name': 'BHIM KUMARI TAMANG', 'fathername': 'NAR BAHADUR TAMANG', 'dob': '2030-05-03', 'citizenship_no': '2026/8364', 'search_list': ['name', 'fathername', 'dob', 'citizenship_no']}
        # item = {'account_nature': 'individual', 'name': 'ROSHAN SHRESTHA', 'fathername': 'ASHOK SHRESTHA', 'dob': '16/04/2035', 'citizenship_no': '19-028', 'search_list': ['name', 'fathername', 'dob', 'citizenship_no']}
        # item = {'account_nature': 'individual', 'name': 'ROSHAN SHRESTHA', 'fathername': 'ASHOK SHRESTHA', 'dob': '16/04/2035', 'pan_no': '603102202', 'search_list': ['name', 'fathername', 'dob', 'pan_no']}
        # item = {'account_nature': 'individual', 'name': 'PANKAJ KUMAR', 'fathername': 'HARDEV CHAUDHARI', 'dob': '16/04/2035', 'indianembassy_no': '19-028', 'search_list': ['name', 'fathername', 'dob', 'indianembassy_no']}
        ...
        try:
            ...
            if str(item['account_nature']).lower() == 'institutions':
            ...
            self.file.combine_columns_by_addition(columns=new_columns_list)
            ...
            temp_result = self.file.get_data_by_value_limit('similarity', min_value, required_columns, add_colum=True, column_data=column_data)
            if not temp_result:
